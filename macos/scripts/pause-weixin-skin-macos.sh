...
record_pause_error() {
  ...
  finish_client_operation "${PORT:-9342}" error "暂停失败，原状态可能未改变" \
...
if [ "$REMOVED" = "true" ]; then
  printf 'Codex WeChat Skin paused (skin removed; control connection %s). Port %s may still be in debug mode.\n' \
    "$([ "$KEEP_CONTROL_WATCHER" = "true" ] && printf retained || printf unavailable)" "$PORT"
elif codex_is_running; then
  printf 'Codex WeChat Skin paused (injector stopped). Live remove skipped: CDP on port %s not verified.\n' "$PORT"
else
  printf 'Codex WeChat Skin paused (ChatGPT is not running).\n'
fi
