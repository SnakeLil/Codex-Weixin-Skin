...
on run argv
  ...
  display dialog promptText buttons {"取消", okLabel} default button okLabel with title "Codex WeChat Skin"
end run
APPLESCRIPT
}
...
THEME_NAME=""
PORT="9342"
if [ -x "$SCRIPT_DIR/status-weixin-skin-macos.sh" ]; then
  ...
  done < <("$SCRIPT_DIR/status-weixin-skin-macos.sh" 2>/dev/null)
fi
...
"$SCRIPT_DIR/start-weixin-skin-macos.sh" --restart-existing >>"$LOG_OUT" 2>&1
code=$?
