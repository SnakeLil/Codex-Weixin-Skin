...
// WeChat Skin CSS auto-adapts to light/dark via data-weixin-shell.
...
async function acquireConfigLock() {
  const lockPath = `${configPath}.weixin-skin.lock`;
  ...
  while (true) {
    ...
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error("Another WeChat Skin config operation is still running; try again shortly.");
      }
...
async function main() {
  ...
  // These layout guards protect the REWRITE path (replaceSetting works line by
  // line). They must not block install, which only backs up appearance keys and
  // never rewrites config.toml, so an unrelated multiline array elsewhere in the
  // user's config is harmless. Run them only right before an actual write.
