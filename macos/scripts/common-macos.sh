...
if [ -z "${HOME:-}" ]; then
  ...
  [ -n "$HOME" ] || { printf 'Codex WeChat Skin: could not resolve the current macOS home directory.\n' >&2; exit 1; }
  export HOME
fi
...
INSTALL_ROOT="$HOME/.codex/codex-weixin-skin-studio"
STATE_ROOT="$HOME/Library/Application Support/CodexWeixinSkinStudio"
...
CODEX_APP_JOB_LABEL="com.openai.codex-weixin-skin-studio.app"
INJECTOR_JOB_LABEL="com.openai.codex-weixin-skin-studio.injector"
...
SKIN_VERSION="1.0.0"
WEIXIN_SKIN_VALIDATED_RUNTIME_PID=""
WEIXIN_SKIN_VALIDATED_RUNTIME_BUNDLE=""
WEIXIN_SKIN_VALIDATED_RUNTIME_EXE=""
WEIXIN_SKIN_VALIDATED_RUNTIME_NODE=""
...
on run argv
  display notification (item 1 of argv) with title "Codex WeChat Skin"
end run
APPLESCRIPT
}
...
alert_user() {
  ...
  /usr/bin/osascript - "$message" <<'APPLESCRIPT' >/dev/null 2>&1 || true
on run argv
  display alert "Codex WeChat Skin" message (item 1 of argv)
end run
APPLESCRIPT
}
...
seed_bundled_presets() {
  ...
  local retired
  for retired in \
    preset-gothic-void-crusade preset-arina-hashimoto; do
...
remember_validated_runtime_identity() {
  WEIXIN_SKIN_VALIDATED_RUNTIME_PID="$$"
  WEIXIN_SKIN_VALIDATED_RUNTIME_BUNDLE="$CODEX_BUNDLE"
  WEIXIN_SKIN_VALIDATED_RUNTIME_EXE="$CODEX_EXE"
  WEIXIN_SKIN_VALIDATED_RUNTIME_NODE="$NODE"
}
...
recorded_injector_process_matches() {
  ...
  # particular, `--port 93410` must never satisfy a saved `9342` identity.
...
stop_recorded_injector() {
  ...
  if ! pid="$(state_field injectorPid 2>/dev/null)" || [ -z "${pid:-}" ]; then
    printf 'WeChat Skin state is damaged or missing its injector PID; state was preserved.\n' >&2
    return 1
  fi
  ...
  case "$pid" in
    *[!0-9]*|??????????*)
      printf 'Recorded WeChat Skin injector PID is invalid; state was preserved.\n' >&2
      return 1
      ;;
  esac
  ...
  case "$saved_port" in
    ''|*[!0-9]*)
      printf 'Recorded WeChat Skin injector port is missing or invalid; state was preserved.\n' >&2
      return 1
      ;;
  esac
  [ "$saved_port" -ge 1024 ] && [ "$saved_port" -le 65535 ] || {
    printf 'Recorded WeChat Skin injector port is out of range; state was preserved.\n' >&2
    return 1
  }
  if [ -z "$saved_start" ] || [ -z "$saved_node" ] || [ -z "$saved_injector" ]; then
    printf 'Recorded WeChat Skin injector identity is incomplete; state was preserved.\n' >&2
    return 1
  fi
  ...
  if recorded_injector_process_matches "$pid" "$saved_start" "$saved_node" "$saved_injector" "$saved_port"; then
    printf 'Could not stop the recorded WeChat Skin injector (PID %s).\n' "$pid" >&2
    return 1
  fi
  return 0
}
...
ensure_node_runtime() {
  if [ "$WEIXIN_SKIN_VALIDATED_RUNTIME_PID" = "$$" ] \
    && [ -n "$WEIXIN_SKIN_VALIDATED_RUNTIME_NODE" ] \
    && [ "${NODE:-}" = "$WEIXIN_SKIN_VALIDATED_RUNTIME_NODE" ] \
    && [ "${CODEX_BUNDLE:-}" = "$WEIXIN_SKIN_VALIDATED_RUNTIME_BUNDLE" ] \
    && [ "${CODEX_EXE:-}" = "$WEIXIN_SKIN_VALIDATED_RUNTIME_EXE" ]; then
    return 0
  fi
  discover_codex_app
  require_signed_node_runtime
